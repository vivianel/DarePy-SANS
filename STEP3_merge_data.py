...
files = {'files':[], 'det':[],'idx':[], 'name':[], 'q':[], 'I':[], 'qc':[], 'Ic':[], 'sigma':[], 'sigmac':[], 'a':[], 'b':[]}
for kk in list_dir:
    if kk[0:3] == 'det':
        det = kk[4:]
...
for kk in range(0, len(files['files'])):
    temp = len(files['files'][kk]['name'])
    if temp > large:
        large = kk



for jj in range(0, len(files['files'][large]['name'])):
    name_0 = str(files['files'][large]['name'][jj])
    idx = []
    name_list = []
    for kk in range(0, len(files['det'])):
        if name_0 in files['files'][kk]['name']:
            name_find = files['files'][kk]['name'].index(name_0)
            idx.append(name_find)
            name_list.append((files['files'][kk]['name'][name_find]))
        else:
            idx.append('--')
            name_list.append('--')
    files['idx'].append(idx)
    files['name'].append(name_list)
...
for kk in range(0, len(files['idx'])):
    ...
    for jj in range(0, len(files['idx'][kk])):
        ...
        if ii == '--':
            ...
            file_name = path_rad_int + 'rad_integ_' + files['files'][jj]['scan'][ii] + '_' + files['files'][jj]['name'][ii] + '_' +files['det'][jj] + 'm.dat'           
            #print(file_name)
            file_dat = np.genfromtxt(file_name, delimiter=',')
            q.append(file_dat[:,0])
            I.append(file_dat[:,1])
            sigma.append(file_dat[:,2])
            #plt.loglog(file_dat[:,0], file_dat[:,1], 'o', ms = 2, label = files['files'][jj]['scan'][ii] + '  ' + files['files'][jj]['name'][ii] + ' ' + files['det'][jj])
            plt.errorbar(file_dat[:,0], file_dat[:,1], yerr =file_dat[:,2], lw = 0.3, marker = 'o',  ms = 2, label = files['files'][jj]['scan'][ii] + '  ' + files['files'][jj]['name'][ii] + ' ' + files['det'][jj])
    plt.xlabel(r'Scattering vector q [$\AA^{-1}$]')
    plt.xscale('log')
    plt.yscale('log')
    plt.ylabel(r'Intensity I [cm$^{-1}$]')
    plt.title('Sample: '+  files['name'][kk][jj])
    plt.legend()
    file_name = path_merged_fig + files['name'][kk][jj] + '_all_det_dist' + '.jpeg' 
    ...
    for kk in range(0, len(files['idx'])):
        ...
        if len(files['q'][kk][0]) != 0 and len(files['q'][kk][1]) != 0:
            if len(files['q'][kk][0]) != 0:
                plt.plot(files['q'][kk][0], files['I'][kk][0], 'ok', label = 'base', ms = 2)
            ...
            plt.plot(files['q'][kk][1], files['I'][kk][1], 'r+', label = 'original', ms = 1)
            files['Ic'][kk][1] = files['I'][kk][1]*mean_01/mean_10
            files['sigmac'][kk][1] = files['sigma'][kk][1]*mean_01/mean_10
            plt.plot(files['qc'][kk][1], files['Ic'][kk][1], 'or', label = 'x ' + str(round(mean_01/mean_10,4)), ms = 2)
            
            
        if len(files['q'][kk][1]) != 0 and len(files['q'][kk][2]) != 0:
            if len(files['q'][kk][0]) == 0 and len(files['q'][kk][1]) != 0:
                plt.plot(files['q'][kk][1], files['I'][kk][1], 'ok', label = 'base', ms = 2)
            ...
            idx3 = int(idx3)
            mean_12 = np.median(files['Ic'][kk][1][0:idx3])
            mean_21 = np.median(files['I'][kk][2][idx2:-1])
            plt.plot(files['q'][kk][2], files['I'][kk][2], 'b+', label = 'original', ms = 1)
            files['Ic'][kk][2] = files['I'][kk][2]*mean_12/mean_21
            files['sigmac'][kk][2] = files['sigma'][kk][2]*mean_12/mean_21
            plt.plot(files['qc'][kk][2], files['Ic'][kk][2], 'ob', label = 'x ' + str(round((mean_12/mean_21)/(mean_01/mean_10) ,4)), ms = 2)
...
for kk in range(0, len(files['idx'])):
    ...
    file_name = path_merged_txt + 'merged_'  + name + '.dat' 
