...
def plot_integ(path_analysis, ScanNr, Frame, plot_radial, plot_azimuthal, save_fig, q_range):

    file_name = os.path.join(path_analysis, 'result.npy')
    with open(file_name, 'rb') as handle:
        result = pickle.load(handle)

    file_name = os.path.join(path_analysis, 'config.npy')
    with open(file_name, 'rb') as handle:
    ...
    if plot_azimuthal == 1:
        ...
        range_angle = list(range(30, 390, 30))
